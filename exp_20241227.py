        ...
        if geodesic_dist <= trajectory_dist_threshold:
            ...
            def astar_weight(a, b, attrs):
                ...
                if not polygon.contains(Point(*a[::-1])) or not polygon.contains(Point(*b[::-1])):
                    return None
                if graph.nodes[a].get("draught", ship_draught) < ship_draught or graph.nodes[b].get("draught",
...
def process_all_trajectories(graph, input_dir, output_dir, neighbor_radius, trajectory_dist_threshold, curve_buffer_dist, straight_buffer_dist, bezier_factor):
